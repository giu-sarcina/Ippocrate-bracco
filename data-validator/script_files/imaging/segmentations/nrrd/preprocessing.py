...
LOG_FILENAME = "nrrdseg_preprocessing_error.log"

class NrrdSegReorientPreprocessor:
    ...
    def check_and_reorient_nrrdseg(self):
        """
        Iterates over all segmentation files in the dataset, checking if they are in LPS orientation,
        and converting them to LPS if needed. Stores the reoriented data in a dictionary.
    
        Returns:
        - lps_segmentations (dict): Dictionary with keys as (patient_folder, series_folder) and 
                                     values as (nrrd_data, updated_space_directions).
        - reorient_message (str): Message indicating if reorientation occurred.
        """
                ...
                if os.path.isdir(series_folder_path):
                    ...
                    for subfolder in os.listdir(series_folder_path):
                        ...
                        if os.path.isdir(subfolder_path):
                            ...
                            break  # Assume the first subfolder is the segmentation folder
                    ...
                    # CONSIDER THE POSSIBILITY TO HAVE MORE THAN ONE NRRD SEG FILE 
                    # Find the NRRD file in the segmentation folder
                    ...
                    for nrrd_filename in nrrd_files:
                        ...
                        print(f"Current orientation for {nrrd_filename}: {current_orientation} (desired orientation: {ref_coordinate_sys})")
    
                        # If already in LPS, store the data as-is
                        if current_orientation != "LPS":
                            nrrd_data, _, _ = self.reorient_to_lps(nrrd_data, nrrd_header, current_orientation, ref_coordinate_sys)
                            reoriented_occurred = True
                            print(f"Reoriented {nrrd_filename} to LPS.")
                        ...
                        if single_mask and multi_segment and nrrd_data.ndim == 3:
                            label = nrrd_filename[:-5] if nrrd_filename.endswith('.nrrd') else nrrd_filename
                            npy_filename = f"seg_{new_series_name2}_segment{label}.npy"
                        else:
                            npy_filename = f"seg_{new_series_name2}.npy"

                        nrrd_data = nrrd_data.astype(np.int16)
                        save_path = os.path.join(seg_out_path, npy_filename)
                        nrrd_data = np.transpose(nrrd_data, (2, 1, 0))  # Z, Y, X -> Slices first
                        np.save(save_path, nrrd_data)
    ...
    def single_to_multi_label_conversion(self, multiple_seg_file_flag): # 46
        """
        Converts multiple single-label NRRD files into a single multi-label NRRD file.
    ...
    def generate_NrrdSegReorientPreprocess_final_report(self, reorient_message, single_to_multilabel_message, removal_message, final_message_resize, no_equal_seg_message, npy_dim_success_message):
        """
        Generate a comprehensive final report for NrrdSegReorientTransform process.
        ...
        report_filename = "4.NrrdSegReorientPreprocess_final_report.txt"
        ...
        report_content.append("NRRD segmentation reorientation and transformation report:\n")
        ...
        print(f"NrrdSegReorientPreprocess final report saved to {report_filepath}")

    
def run_nrrdseg_reorient_preprocessor(protocol, local_config, mapping_file, series_group_name):
    ...
    nrrdseg_preprocessor = NrrdSegReorientPreprocessor(protocol, local_config, mapping_file, num_image_patients, num_slices_group, total_series, total_slices, series_group_name)

    series_progress_state =  load_state(nrrdseg_preprocessor.series_progress_file)
    ...
    if last_phase_done < 48:
        ...
        print("Running check_and_reorient_nrrdseg function...")
        try:
            reorient_message = nrrdseg_preprocessor.check_and_reorient_nrrdseg()
        except Exception as e:
            log_error(input_dir, "check_and_reorient_nrrdseg", e, LOG_FILENAME)
            print(f"An unexpected error occurred during check_and_reorient_nrrdseg. Details were written to {os.path.join(input_dir, LOG_FILENAME)}.")
            raise 

        print("Running single_to_multi_label_conversion function...")
        try:
            single_to_multilabel_message = nrrdseg_preprocessor.single_to_multi_label_conversion(image_state.get("multiple_segmentation_flag"))
        except Exception as e:
        ...
        try:
            removal_message = nrrdseg_preprocessor.discard_black_seg_slices()
        except Exception as e:
        ...
        try:
            final_message_resize = nrrdseg_preprocessor.resize_seg()
        except Exception as e:
        ...
        try:
            no_equal_seg_message = nrrdseg_preprocessor.check_seg_replicates_after_processing()

            series_state["last_successful_phase"] = 47
            series_progress_state[series_group_name] = series_state
            save_state(series_progress_state, nrrdseg_preprocessor.series_progress_file)
        except Exception as e:
        ...
        try:
            npy_dim_success_message = nrrdseg_preprocessor.validate_npy_dimension_match()

            # Phase 48: Final step of nrrdseg preprocessing; all prior validation assumed complete.
        ...
        print("Running generate_NrrdSegReorientPreprocess_final_report function...")
        try:
            nrrdseg_preprocessor.generate_NrrdSegReorientPreprocess_final_report(reorient_message, single_to_multilabel_message, removal_message, final_message_resize, no_equal_seg_message, npy_dim_success_message)
        except Exception as e:
            log_error(input_dir, "generate_NrrdSegReorientPreprocess_final_report", e, LOG_FILENAME)
            print(f"An unexpected error occurred during generate_NrrdSegReorientPreprocess_final_report. Details were written to {os.path.join(input_dir, LOG_FILENAME)}.")
            raise 
