...
LOG_FILENAME = "niftiseg_validation_error.log"

class NiftiSegValidator:
    ...
    def check_num_slices_nifti_seg(self): # NB: se ci sono più segmentazioni NIfTI per serie lo eseguo su tutte 
        """
        Checks if the number of segmentation slices in each NIfTI file in the segmentation folder(s)
        matches the number of volume slices in the corresponding series folder.
        If there are multiple NIfTI segmentation files for a series, it processes each one.
                ...
                if os.path.isdir(series_folder_path):
                    if image_group_config["image file format"]["selected"] == ".dcm": 
                        ...
                        num_slices = len(dicom_files)
                         ...
                         # Find NIfTI image files in the series folder 
                         nifti_files_volume = [f for f in os.listdir(series_folder_path) if f.endswith('.nii') or f.endswith('.nii.gz')] # output = list 
                         nifti_image_path = os.path.join(series_folder_path, nifti_files_volume[0])
                        ...
                        # Find NRRD image files in the series folder
                        ...
                        raise ValueError(f"Invalid image file format specified in protocol: {self.protocol['image']['image file format']['selected']}.")
 

                    # Find the unique folder in the series folder (which should contain the NIfTI file)
                    ...
                    for subfolder in os.listdir(series_folder_path):
                        ...
                        if os.path.isdir(subfolder_path):
                            ...
                            break  # We only need to select the first (and only) subfolder containing the NIfTI file
                            
                    # CONSIDERA LA POSSIBILITA' DI AVERE PIU' FILE DI SEGMENTAZIONE
                    # Assuming there is exactly one NIfTI file in the segmentation folder
                    ...
                    for nifti_filename in nifti_files:
                        ...
                        # Check NIfTI dimensions
                        #nifti_size = nifti_data.shape[:2]  # Assuming the first two dimensions are [height, width]
                        nifti_num_slices = nifti_data.shape[2]  # Assuming third dimension is the number of slices
                    
                        # Check that the number of slices matches between NIfTI and DICOM
                        if nifti_num_slices != num_slices:
                            discrepancy_info += f"  - Number of slices mismatch for patient {patient_folder}, series {series_folder}, segmentation file {nifti_filename}: segmentations ({nifti_num_slices}) vs volumes ({num_slices})\n"
                            num_errors += 1
                            has_error = True
        ...
        if has_error:
            ...
            success_size_niftiseg = "All segmentation files match the slice count of their corresponding volumes."
            return success_size_niftiseg, multiple_segmentation_flag
        

    def validate_segment_code_conformity(self, multiple_seg_file_flag): # NB: se ci sono più segmentazioni NIfTI per serie lo eseguo su tutte 
        """
            ...
            if os.path.isdir(patient_folder_path):
                ...
                if os.path.isdir(series_folder_path):
                    ...
                    for subfolder in os.listdir(series_folder_path):
                        ...
                        if os.path.isdir(subfolder_path):
                            segmentation_folder = subfolder_path
                            break  # Select the first (and only) subfolder containing the NIfTI file
                    ...
                    if (seg_input_format == "multi-mask") or (seg_input_format == "single-mask" and not multiple_seg_file_flag):
                        # Load the NIfTI file
                        nifti_files = [f for f in os.listdir(segmentation_folder) if f.endswith('.nii') or f.endswith('.nii.gz')]
                        nifti_file_path = os.path.join(segmentation_folder, nifti_files[0])
                        nifti_img = nib.load(nifti_file_path)
                        nifti_data = nifti_img.get_fdata()
    
                        # Check unique values in the NIfTI data
                        ...
                        if unexpected_values:
                            code_errors_content += (f"  - Unexpected segmentation codes in NIfTI for patient {patient_folder}, "
                    ...
                    if seg_input_format == "multi-mask":
                        ...
                        if len(filtered_unique_values) != segments_number:
                            count_errors_content += f"  - Number of unique values ({len(filtered_unique_values)}) in NIfTI segmentation array does not match protocol segments_number ({segments_number}) for patient {patient_folder}, series {series_folder}\n"
                            num_errors += 1
                            has_error = True
    ...
    def single_label_seg_3Dshape_check(self, multiple_segmentation_flag): #45
        """
        Checks for inconsistent shapes among multi-dimensional (NIfTI) segmentations in the input directory structure.
        ...
        for patient_folder in patient_folders:
            ...
            if os.path.isdir(series_folder_path):
                ...
                nifti_files = [f for f in os.listdir(segmentation_folder)
                ...
                for nifti_filename in nifti_files:
                    ...
                    if reference_shape is None:
                        ...
                        shape_inconsistencies[key].append((nifti_filename, seg_array.shape))
    ...
    def check_pixel_overlap_single_label_seg(self, multiple_segmentation_flag):
        ...
        for patient_folder in patient_folders:
            ...
            for nifti_filename in nifti_files:
                ...
                current_file_key = f"{patient_folder}/{series_folder}/{nifti_filename}"
    ...
    def generate_NiftiSegValidation_final_report(self, success_size_niftiseg, segment_code_conformity_message, shape_inconsistency_message, overlapping_pixel_message):
        ...
        final_report_lines = [
            ...
            "",
            "Final report on NIfTI segmentation conformity checks:",
            "",
            "Size check:",
            f"- {success_size_niftiseg}",
            "",
        ...
        report_filename = os.path.join(self.output_directory_report, "3.NiftiSegValidation_final_report.txt")
        ...
        print("3.NiftiSegValidation final report has been generated.")


def run_niftiseg_validator(protocol, local_config, mapping_file, series_group_name):
    ...
    niftiseg_validator = NiftiSegValidator(protocol, local_config, mapping_file, num_image_patients, num_slices_group, total_series, total_slices, series_group_name)

    series_progress_state =  load_state(niftiseg_validator.series_progress_file)
    ...
    if last_phase_done < 41:
        print("Running check_num_slices_nifti_seg function...") # check phase 41
        try:
            image_state["success_size_niftiseg"], image_state["multiple_segmentation_flag"] = niftiseg_validator.check_num_slices_nifti_seg()
            save_state(image_state, niftiseg_validator.state_file)  # Save updated state

            # Update series progress state
            series_state["last_successful_phase"] = 41
            series_progress_state[series_group_name] = series_state
            save_state(series_progress_state, niftiseg_validator.series_progress_file)
        except Exception as e:
            log_error(input_dir, "check_num_slices_nifti_seg", e, LOG_FILENAME)
            print(f"An unexpected error occurred during check_num_slices_nifti_seg. Details were written to {os.path.join(input_dir, LOG_FILENAME)}.")
            raise

        # NOTE: Phases 42 and 43 intentionally skipped (not applicable to NIfTI)

    if last_phase_done < 44:
        print("Running validate_segment_code_conformity function...") # check phase 44
        try:
            image_state["segment_code_conformity_message"] = niftiseg_validator.validate_segment_code_conformity(image_state.get("multiple_segmentation_flag"))
            save_state(image_state, niftiseg_validator.state_file)
    ...
    if last_phase_done < 45:
        ...
        try:
            image_state["shape_inconsistency_message"] = niftiseg_validator.single_label_seg_3Dshape_check(image_state.get("multiple_segmentation_flag"))
            save_state(image_state, niftiseg_validator.state_file)
    ...
    if last_phase_done < 46:
        ...
        try:
            image_state["overlapping_pixel_message"] = niftiseg_validator.check_pixel_overlap_single_label_seg(image_state.get("multiple_segmentation_flag"))
            save_state(image_state, niftiseg_validator.state_file)
        ...
        print("Running generate_NiftiSegValidation_final_report function...")
        try:
            niftiseg_validator.generate_NiftiSegValidation_final_report(
                    image_state.get("success_size_niftiseg"), 
            ...
            log_error(input_dir, "generate_NiftiSegValidation_final_report", e, LOG_FILENAME)
            print(f"An unexpected error occurred during generate_NiftiSegValidation_final_report. Details were written to {os.path.join(input_dir, LOG_FILENAME)}.")
            raise 
