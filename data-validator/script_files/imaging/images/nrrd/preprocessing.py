...
LOG_FILENAME = "nrrd_preprocessing_error.log"

class NrrdReorientPreprocessor:
    ...
    def determine_orientation(self, header_nrrd):
        """
        Determine the orientation of the image based on the NRRD header information.
    
        Parameters:
        header_nrrd (dict): A dictionary containing NRRD header metadata including 'space' and 'space directions'.
    
        Returns:
        str: A string representing the orientation (e.g., 'LPS', 'RAS', 'LAS').
        """
        orientation = []
    
        # Validate the presence of required keys
        if 'space' not in header_nrrd or 'space directions' not in header_nrrd:
            raise KeyError("Header must contain 'space' and 'space directions' keys.")
        
        space = header_nrrd['space']
        space_directions = header_nrrd['space directions']
    
        # Validate space directions
        if space_directions.shape[0] != 3 or space_directions.shape[1] != 3:
            raise ValueError("Space directions must be a 3x3 matrix.")
        
        # Extract scaling factors from the main diagonal
        scaling_factors = np.diag(space_directions)
        
        # Define mapping based on the space type
        if space == 'LPS' or space == 'left-posterior-superior' or space == 'LPST' or space == 'left-posterior-superior-time':
            direction_map = {'pos_x': 'L', 'neg_x': 'R', 'pos_y': 'P', 'neg_y': 'A', 'pos_z': 'S', 'neg_z': 'I'}
            coordinate_system = 'LPS'
        elif space == 'RAS' or space == 'right-anterior-superior' or space == 'RAST' or space == 'right-anterior-superior-time':
            direction_map = {'pos_x': 'R', 'neg_x': 'L', 'pos_y': 'A', 'neg_y': 'P', 'pos_z': 'S', 'neg_z': 'I'}
            coordinate_system = 'RAS'
        elif space == 'LAS' or space == 'left-anterior-superior' or space == 'LAST' or space == 'left-anterior-superior-time':
            direction_map = {'pos_x': 'L', 'neg_x': 'R', 'pos_y': 'A', 'neg_y': 'P', 'pos_z': 'S', 'neg_z': 'I'}
            coordinate_system = 'LAS'
        else:
            raise ValueError("Unsupported space type. Must be 'LPS', 'RAS', or 'LAS'.")
            
        # Determine orientation based on scaling factors
        for i, factor in enumerate(scaling_factors):
            if i == 0:  # x-axis
                orientation.append(direction_map['pos_x'] if factor > 0 else direction_map['neg_x'])
            elif i == 1:  # y-axis
                orientation.append(direction_map['pos_y'] if factor > 0 else direction_map['neg_y'])
            elif i == 2:  # z-axis
                orientation.append(direction_map['pos_z'] if factor > 0 else direction_map['neg_z'])
        
        return ''.join(orientation), coordinate_system
    

    def create_reorientation_matrix(self, current_orientation, ref_coordinate_sys):
        # Identity matrix (starts as if already LPS)
        reorient_matrix = np.eye(3)
        
        # Mapping based on the current orientation to LPS
        orientation_mapping = {
            'L': 1,  # L stays L
            'R': -1, # R flips to L
            'A': -1, # A flips to P
        ...
        if ref_coordinate_sys in ['LPS', 'RAS', 'LAS']:
            # Apply orientation mapping based on the current orientation
            reorient_matrix[0, 0] *= orientation_mapping[current_orientation[0]]  # x-axis
            reorient_matrix[1, 1] *= orientation_mapping[current_orientation[1]]  # y-axis
            reorient_matrix[2, 2] *= orientation_mapping[current_orientation[2]]  # z-axis
        else:
            raise ValueError(f"Unsupported reference coordinate system: '{ref_coordinate_sys}'. Must be 'LPS', 'RAS', or 'LAS'.")
    
        return reorient_matrix
    

    def reorient_to_lps(self, image_data, header_nrrd, current_orientation, ref_coordinate_sys):
        """
        Reorient image data to LPS (Left-Posterior-Superior) orientation based on the NRRD header information.
    
        Parameters:
        image_data (numpy.ndarray): The original image data to be reoriented.
        header_nrrd (dict): The NRRD header information containing metadata, including 'space directions'.
    
        Returns:
        tuple: A tuple containing:
            - reoriented_image_data (numpy.ndarray): The reoriented image data (if the reorientation is performed).
            - reoriented_space_directions (numpy.ndarray): The new space directions after reorientation (if performed).
            - updated_header_nrrd (dict): The updated header with new space directions.
        """
        ...
        space_directions = header_nrrd['space directions']
        current_space_origin = np.array(header_nrrd['space origin'])
        
        # Validate space directions
        if space_directions.shape[0] != 3 or space_directions.shape[1] != 3:
            raise ValueError("Space directions must be a 3x3 matrix.")
    
        # Initialize reoriented data with the original image data
        reoriented_image_data = image_data
 
        # Create reorientation matrix
        reorient_matrix = self.create_reorientation_matrix(current_orientation, ref_coordinate_sys)
        
        # Reorient space directions (physical space representation)
        reoriented_space_directions = np.dot(reorient_matrix, space_directions.T).T

        # Update the header with the new space directions
        header_nrrd['space directions'] = reoriented_space_directions

        # Determine the size of the image along each axis
        image_shape = image_data.shape
        sizes = {axis: size for axis, size in zip(['x', 'y', 'z'], image_shape)}

        # Adjust the space origin for flipped axes
        for i, axis in enumerate(['x', 'y', 'z']):
            if (current_orientation[i] == 'R' and axis == 'x') or \
               (current_orientation[i] == 'A' and axis == 'y') or \
               (current_orientation[i] == 'I' and axis == 'z'):
                # If the axis is flipped, recalculate the translation term
                voxel_size = space_directions[i, i]  # Size of the voxel in the direction of the axis
                new_translation = current_space_origin[i] + (sizes[axis] - 1) * voxel_size
                current_space_origin[i] = new_translation

        # Update the header with the new space origin
        header_nrrd['space origin'] = current_space_origin
        
        # Determine which axes to flip
        flip_axes = []
        if current_orientation[0] == 'R':
            flip_axes.append(0)  # Flip x-axis
        if current_orientation[1] == 'A':
            flip_axes.append(1)  # Flip y-axis
        if current_orientation[2] == 'I':
            flip_axes.append(2)  # Flip z-axis
            
        # Apply flips to the image data
        if flip_axes:
            reoriented_image_data = np.flip(image_data, axis=tuple(flip_axes))
        
        return reoriented_image_data, reoriented_space_directions, header_nrrd
    

    def check_and_reorient_nrrdvolumes(self):
        """
        Iterates over all volume NRRD files in the dataset, checking if they are in LPS orientation,
        and converting them to LPS if needed. Save:
        ...
        reoriented_occurred = False  # Flag to track if any volume was reoriented
        ...
        for orig_patient_id in patient_folders:
            ...
            if os.path.isdir(patient_folder_path):
                ...
                if os.path.isdir(series_folder_path):
                    ...
                    # Find the NRRD file in the series folder
                    nrrd_files = [f for f in os.listdir(series_folder_path) if f.endswith('.nrrd')]
                    if not nrrd_files:
                        print(f"No NRRD files found in {series_folder_path}. Skipping.")
                        continue
                    nrrd_file_path = os.path.join(series_folder_path, nrrd_files[0])
                    nrrd_data, nrrd_header = nrrd.read(nrrd_file_path)
    
                    # Check the current orientation
                    current_orientation, ref_coordinate_sys = self.determine_orientation(nrrd_header)
                    print(f"Current orientation for {nrrd_files[0]}: {current_orientation} (desired orientation: {ref_coordinate_sys})")
    
                    # Check the Superior-Inferior (S/I) direction flag
                    si_flag = current_orientation[2] == 'I'
    
                    if current_orientation != "LPS":
                        # Reorient to LPS
                        nrrd_data, reoriented_space_directions, _ = self.reorient_to_lps(nrrd_data, nrrd_header, current_orientation, ref_coordinate_sys)
    
                        # Update flag if reorientation occurred
                        reoriented_occurred = True
                    else:
                        reoriented_space_directions = nrrd_header['space directions']
                    ...
                    np.save(tensor_path, nrrd_data)

                    # Save metadata as .json
                    metadata = {
                        "patient_folder": new_patient_id,
                        "series_folder": new_series_name,
                        "reoriented_space_directions": reoriented_space_directions.tolist(),
    ...
    def process_array(self):
        ...
        if discarded_slices:
            ...
            if new_rows:
                ...
                print(f"{len(new_rows)} new discarded slices written to {csv_path}") #!! Path del container, si può non cambiare
            else:
        ...
        if num_discarded_slices > 0:
            processing_summary.append(f"- discarded_slices.csv file has been created/updated in the directory: {self.host_images_dir}") #!! Path del container, deve essere quello locale
    ...
    def check_image_replicates_after_processing(self): 
        ...
        if grouped_series or flagged_series_std:
            ...
            print(f"Highly similar slices detected. "
                             f"See the detailed report at: {host_report_path}") #!! Path del container, si può non cambiare
                    ...
                    for file_name in os.listdir(series_path):
                        if file_name.startswith('MIP'):
                            ...
                            try:
                                ...
                                print(f"Saved feature vector to {feature_file_path}") #!! Path del container, si può non cambiare
                                
                            except Exception as e:
                                print(f"Failed to process image {img_path}: {e}") #!! Path del container, si può non cambiare
    ...
    def generate_NrrdReorientPreprocess_final_report(self, reoriented_occurred, final_processing_summary, final_message_resize, replicate_slice_message, warnings_replicates, final_message_npy_and_labels, message_from_npy_to_image, message_series_alignment, total_misaligned_slices, mip_message, message_extract_feat):
        # Get the current datetime
        current_datetime = datetime.now()
        formatted_datetime = current_datetime.strftime("%Y-%m-%d %H:%M:%S")
        
        # Define the report filename and path
        report_filename = "2.NrrdReorientPreprocess_final_report.txt"
        report_filepath = os.path.join(self.output_directory_report, report_filename)
        ...
        report_content.append("NRRD reorientation and preprocessing final report:\n")
        ...
        print(f"Final report saved to {report_filepath}") #!! Path del container, si può non cambiare


def run_nrrd_reorient_preprocessor(protocol, local_config, mapping_file, series_group_name):
    ...
    nrrd_preprocessor = NrrdReorientPreprocessor(protocol, local_config, mapping_file, num_image_patients, series_group_name)

    state = load_state(nrrd_preprocessor.state_file)
    series_progress_state =  load_state(nrrd_preprocessor.series_progress_file)
    ...
    if last_phase_done < 36:
        ...
        print("Running check_and_reorient_nrrdvolumes function...")
        try:
            reoriented_occurred = nrrd_preprocessor.check_and_reorient_nrrdvolumes()
        except Exception as e:
            log_error(input_dir, "check_and_reorient_nrrdvolumes", e, LOG_FILENAME)
            print(f"An unexpected error occurred during check_and_reorient_nrrdvolumes. Details were written to {os.path.join(input_dir, LOG_FILENAME)}.")
            raise 

        print("Running process_array function...")
        try:
            final_processing_summary = nrrd_preprocessor.process_array()
        except Exception as e:
        ...
        try:
            final_message_resize = nrrd_preprocessor.resize_array()
        except Exception as e:
        ...
        try:
            final_message_npy_and_labels = nrrd_preprocessor.save_labels()
        except Exception as e:
        ...
        try:
            message_from_npy_to_image = nrrd_preprocessor.convert_npy_to_image()
        except Exception as e:
        ...
        try:
            mip_message = nrrd_preprocessor.create_and_save_MIP_image()
        except Exception as e:
        ...
        try:
            _, _, message_extract_feat = nrrd_preprocessor.extract_features_from_MIP()
        except Exception as e:
        ...
        try:
            replicate_slice_message, state["num_slices_group"], state["total_series"], state["total_slices"], state["warnings_replicates"], input_state["num_patients_with_image_data"] = nrrd_preprocessor.check_image_replicates_after_processing()
            save_state(state, nrrd_preprocessor.state_file)
            ...
            save_state(series_progress_state, nrrd_preprocessor.series_progress_file)
        except Exception as e:
        ...
        try:
            message_series_alignment, total_misaligned_slices = nrrd_preprocessor.analyze_series_alignment()
        except Exception as e:
        ...
        print("Running generate_NrrdReorientPreprocess_final_report function...")
        try:
            nrrd_preprocessor.generate_NrrdReorientPreprocess_final_report(reoriented_occurred, final_processing_summary, final_message_resize, replicate_slice_message, state.get("warnings_replicates"), final_message_npy_and_labels, message_from_npy_to_image, message_series_alignment, total_misaligned_slices, mip_message, message_extract_feat)
        except Exception as e:
            log_error(input_dir, "generate_NrrdReorientPreprocess_final_report", e, LOG_FILENAME)
            print(f"An unexpected error occurred during generate_NrrdReorientPreprocess_final_report. Details were written to {os.path.join(input_dir, LOG_FILENAME)}.")
            raise 
