...
LOG_FILENAME = "nifti_validation_error.log"
...
class NiftiValidator:
    ...
    def check_slice_dimensions_nifti(self):
        ...
        for patient_folder in patient_folders:
            ...
            nifti_files = [f for f in os.listdir(series_path) if f.endswith(('.nii', '.nii.gz'))]
            ...
            for nifti_file in nifti_files:
                ...
                try:
                    ...
                    data = img.get_fdata()
                    num_slices = data.shape[2] if len(data.shape) >= 3 else 1  # 3D or 4D support
                    ...
                    for i in range(num_slices):
                        ...
                        if rows < target_rows or cols < target_cols:
                            msg = (
                                ...
                                f"File: {nifti_file}, Slice {i}: {rows}x{cols}"
                            )
                    ...
                    raise ValueError(f"Error reading NIfTI file {nifti_path}: {e}") #!! Path del container
    ...
    def generate_NiftiValidation_final_report(self, slice_dim_message):
        ...
        report_filename = os.path.join(self.output_directory_report, "1.NiftiValidation_final_report.txt") 
        ...
        print("NiftiValidation final report has been generated.")


def run_nifti_validator(protocol, local_config, mapping_file, series_group_name):
    ...
    # Create an instance of NiftiValidator
    nifti_validator = NiftiValidator(protocol, local_config, mapping_file, num_image_patients, series_group_name)

    state = load_state(nifti_validator.state_file)
    series_progress_state =  load_state(nifti_validator.series_progress_file)
    ...
    if last_phase_done < 35:
        print("Running check_slice_dimensions_nifti function...") # check phase 35
        try:
            state["slice_dim_message"] = nifti_validator.check_slice_dimensions_nifti()
            save_state(state, nifti_validator.state_file)  # Save updated state

            series_state["last_successful_phase"] = 35
            series_progress_state[series_group_name] = series_state
            save_state(series_progress_state, nifti_validator.series_progress_file)
        except Exception as e:
            log_error(input_dir, "check_slice_dimensions_nifti", e, LOG_FILENAME)
            print(f"An unexpected error occurred during check_slice_dimensions_nifti. Details were written to {os.path.join(input_dir, LOG_FILENAME)}.")
            raise 

        print("Running generate_NiftiValidation_final_report function...")
        try:
            nifti_validator.generate_NiftiValidation_final_report(state.get("slice_dim_message"))
        except Exception as e:
            log_error(input_dir, "generate_NiftiValidation_final_report", e, LOG_FILENAME)
            print(f"An unexpected error occurred during generate_NiftiValidation_final_report. Details were written to {os.path.join(input_dir, LOG_FILENAME)}.")
            raise 
