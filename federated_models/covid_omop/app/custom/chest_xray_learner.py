...
# Use an existing NVFlare logger name, or create your own
logger = logging.getLogger("FFFFFFFFFFFFFFF")  # Choose a descriptive name
logger.setLevel(logging.DEBUG)  # Match the server logging level

# Log messages
logger.debug("This is a DEBUG message from my learner")
logger.info("This is an INFO message visible on server")
...
class Xray_Chests_Learner(Learner):
    ...
    def _create_datasets(self, fl_ctx: FLContext):
        ...
        if self.train_dataset is None or self.train_loader is None:
            ...
            # Check if dataset is effectively empty (only dummy data)
            if len(self.train_dataset) == 1 and np.all(self.train_dataset.data[0] == 0):
                logging.error(f"Training dataset is empty for client {self.client_id}")
                raise ValueError(f"No training data available for client {self.client_id}. Check database connection and data availability.")
        ...
        if self.valid_dataset is None or self.valid_loader is None:
            ...
            # Check if dataset is effectively empty (only dummy data)
            if len(self.valid_dataset) == 1 and np.all(self.valid_dataset.data[0] == 0):
                logging.error(f"Validation dataset is empty for client {self.client_id}")
                raise ValueError(f"No validation data available for client {self.client_id}. Check database connection and data availability.")
    ...
    def train(self, shareable: Shareable, fl_ctx: FLContext, abort_signal: Signal) -> Shareable:
        ...
        if abort_signal.triggered:
            return make_reply(ReturnCode.TASK_ABORTED)
        self.log_info(fl_ctx, f"val_acc_local_model: {acc}")
    ...
    def local_valid(self, valid_loader, abort_signal: Signal, tb_id=None, fl_ctx=None):
        ...
        # TorchMetrics objects (track state across batches)
        accuracy_metric = Accuracy(task="binary").to(self.device)
        cm_metric = ConfusionMatrix(task="binary").to(self.device)
        ...
        with torch.no_grad():
            for _i, (inputs, labels) in enumerate(valid_loader):
                ...
                labels = labels.view(len(labels), 1).float().to(self.device)
            ...
            confusion_matrix = cm_metric.compute().cpu().tolist()

            # Log accuracy to TensorBoard if needed
            if tb_id:
