...
def select_series(data, series_name, target_type=None, select_features=False):
    r"""Select series by series_name from data."""
...
class VectorDataset(Dataset):
    ...
    def __init__(self, data, series_name="series001",target_type=None):
    ...
    def __getitem__(self, index):
        # Load feature vector
        feature_vector_batch = self.data[index]
        feature_vector_batch = select_series(feature_vector_batch, self.series_name, self.target_type, select_features=True)
        batch = []
        for el in feature_vector_batch:
            filename = el[0]
            ...
            for t in target:
                if isinstance(t, str) and t.endswith(".npy"): # array numpy #segmentation mask
                    val_array = np.load(t) 
                    val_array = torch.from_numpy(val_array)  
                    target_values.append(val_array)         
                elif isinstance(t, str) and t.endswith(".txt"):  # string # label 
                    with open(t, "r") as f:
                        val = f.read().strip()                      # stringa 
                        target_values.append(val)
            ...
            batch.append((feature_vector, target_values))
        return batch
