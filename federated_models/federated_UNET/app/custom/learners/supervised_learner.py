...
class SupervisedLearner(Learner):
    ...
    def local_train(
        ...
        Load data pairs from train_loader: image / mask
    ...
    def local_valid(
        ...
        with torch.no_grad():
            ...
            for i, batch_data in enumerate(valid_loader):
                if abort_signal.triggered:
                    return make_reply(ReturnCode.TASK_ABORTED)
                val_images = batch_data[0][0].to(self.device) 
                val_labels = batch_data[0][1].to(self.device) 
                val_labels = torch.where(val_labels == 255, 1, 0)
                # Inference
                print(f"################# Valid batch {i}: inputs shape {val_images.shape}, labels shape {val_labels.shape} ###################")
    ...
    def train(
        ...
        """Typical training task pipeline with potential HE
        ...
        self.log_info(fl_ctx, f"Current/Total Round: {current_round}/{total_rounds}")
